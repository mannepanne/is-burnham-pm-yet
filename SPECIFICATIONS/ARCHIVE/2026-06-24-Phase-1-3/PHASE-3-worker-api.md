...
11. ✅ KV caching implemented with 6-hour TTL
12. ✅ Cron-triggered cache refresh
13. ✅ Manual refresh endpoint with REFRESH_SECRET protection
14. ✅ Full-text article fetching for better judgment (optional upgrade)
15. ✅ Updated judge prompt with expanded definitions and calibration examples
...
### API Tests
- [x] `/api/commentary` returns 200 with valid JSON
- [x] Response has correct structure (probability_pct, one_line, articles)
- [x] Articles have all required fields (title, url, outlet, date, verdict, caption)
- [x] Verdict values are only 'probing', 'fixating', or 'noting'
- [x] At least one article returned when pool is non-empty
- [x] Empty articles array when Stage 1 fails
- [x] Single noting card when Stage 2 fails but Stage 1 succeeded

### Integration Tests
- [x] Frontend correctly renders articles from API response
- [x] Verdict labels display correctly (Fixating on:, Probing, Noted:)
- [x] Probability shows in Odds Desk
- [x] One-line caption shows in Odds Desk
- [x] Articles show outlet, date, title, caption

### Security Tests
- [x] No API keys visible in network tab
- [x] No API keys in source code (only in secrets)
- [x] Worker secrets accessible in Worker environment
- [x] Local dev works with `.dev.vars`
- [x] REFRESH_SECRET protects manual refresh endpoint

### Editorial Integrity Tests
- [x] Test with mock pool containing substantive article → verify `probing` verdict
- [x] Test with mock pool containing frothy article → verify `fixating` verdict
- [x] Test with mock pool containing neutral article → verify `noting` verdict
- [x] Test with mixed pool → verify honest representation (not forced ratio)
- [x] Test with empty pool → verify empty articles array
...
- [x] `/api/commentary` endpoint working correctly
- [x] Both Stage 1 and Stage 2 integrated
...
- Perplexity Sonar API integration (sonar-pro model, ~9 articles pool, week recency filter)
- Claude Haiku-4-5 judge model with comprehensive prompt
- Full-text article fetching as optional upgrade for better judgment
- KV caching with 6-hour TTL via Cloudflare KV namespace (COMMENTARY_CACHE)
- Cron-triggered cache refresh using Worker scheduled() handler
- Manual refresh endpoint at `/api/refresh` with REFRESH_SECRET protection
- Client-side 12-second timeout for commentary API calls
- Fallback to on-demand pipeline if KV cache miss
- Updated judge prompt with expanded definitions for "fixating" (includes inflated drama, tangential hooks)
- Added calibration examples to judge prompt for consistency
- All graceful degradation paths working correctly
- All security measures implemented (no API keys exposed, secrets properly configured)
